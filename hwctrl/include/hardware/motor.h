...
enum class ControlType { None, RPM, Position };

enum class MotorType { None, Vesc, Sabertooth, BMC };

MotorType get_motor_type(boost::string_view);

struct MotorConfig {
  std::string name;
  std::string cmd_topic = "hwctrl/motor_cmd";
  ...
  float gear_reduc = 1.0f;
};

class Motor {
public:
  Motor(ros::NodeHandle nh, MotorConfig const &config);
  ...
  void set_setpoint(ros::Time time, float setpoint = 0.0f,
                    float acceleration = 0.0f);

  inline const std::string &get_name() const { return m_config.name; }
  inline uint32_t get_id() const { return m_config.id; }
  inline MotorType get_motor_type() const { return m_config.m_type; }
  inline ControlType get_control_type() const { return m_config.c_type; }

  inline bool is_online() const { return m_online; }
  inline bool ready_to_update() const { return m_update; }

  inline void disable_update_timer() {
    m_update_timer.stop();
  }

  inline void enable_update_timer() {
    m_update_timer.start();
  }

private:
  void setpoint_callback(boost::shared_ptr<hwctrl::MotorCmd> cmd) {
    set_setpoint(ros::Time::now(), cmd->setpoint, cmd->acceleration);
  }
...
public:
  CanMotor(ros::NodeHandle nh, MotorConfig const &config);
