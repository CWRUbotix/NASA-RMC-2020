...
class Motor {
 public:
  Motor(ros::NodeHandle nh, const std::string& name, uint32_t id,
        MotorType m_type, ControlType c_type,
  ...
  virtual void update(
      ros::Time time) = 0;  // send the setpoint and return sensor data
...
class CanMotor : public Motor {
 public:
  CanMotor(ros::NodeHandle nh, const std::string& name, uint32_t id,
