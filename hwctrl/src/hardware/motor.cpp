...
Motor::Motor(
    ...
    ControlType c_type, ros::Duration update_pd, float accel_setpoint, float max_accel, 
    float max_rpm, float gear_reduc, ros::Duration timeout
)
: m_nh(nh), m_name(name), m_id(id), m_type(m_type), m_control_type(c_type), m_update_pd(update_pd), m_timeout(timeout), 
  m_accel_setpoint(accel_setpoint), m_max_rpm(max_rpm), m_max_accel(max_accel), m_rpm_coef(gear_reduc)
{   
...
CanMotor::CanMotor(
    ...
    ControlType c_type, ros::Duration update_pd, float accel_setpoint, float max_accel, 
    float max_rpm, float gear_reduc, ros::Duration timeout
) : Motor(nh, name, id, m_type, c_type, update_pd, accel_setpoint, max_accel, max_rpm, gear_reduc, timeout), m_can_id(can_id)
{
