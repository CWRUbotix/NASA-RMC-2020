...
void QuadEncoder::can_rx_callback(FramePtr frame) {
    ...
    const uint8_t can_id = (uint8_t)(rx_id & 0xFF); // extract only the id (first 8 bits)
    ...
    switch(cmd) {
        case 0: {
            ...
            ROS_WARN("Recieved error frame from %s (id: %d) - error id: %d", m_name.c_str(), m_id, frame->data[0]);
            break;
        }
        case 1: {
            if(frame->can_dlc < 4) {
