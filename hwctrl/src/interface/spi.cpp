...
int Spi::init(const std::string& fname) {
  ...
  // |= SPI_LSB_FIRST; mode |= SPI_CS_HIGH; mode |= SPI_3WIRE; mode |= SPI_NO_CS;
  // mode |= SPI_READY; multiple possibilities using |
...
int Spi::set_mode(uint32_t spi_mode) {
  if (spi_mode != SPI_MODE_0 && spi_mode != SPI_MODE_1 &&
      spi_mode != SPI_MODE_2 && spi_mode != SPI_MODE_3) {
    spi_mode = SPI_MODE_0;
  }
