...
SensorCalThread::SensorCalThread(ros::NodeHandle nh) : SensorThread(nh){};
...
void SensorCalThread::operator()() {
  ...
  while (ros::ok() && !done) {
    ...
    if (line.compare("imu") == 0) {
      ...
      std::cout << "Saving calibration to file " << cal_file_path << "..."
                << std::endl;
      write_calibration(cal_file_path, cals);
      if (boost::filesystem::exists(cal_file_path.c_str()) &&
          boost::filesystem::file_size(cal_file_path.c_str()) > 0) {
...
void SensorCalThread::calibrate_sensor_with_name(
    boost::string_view name, std::vector<Calibration>& cals) {
  bool found = false;
