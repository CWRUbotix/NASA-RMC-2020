...
CanbusThread::CanbusThread(ros::NodeHandle nh, std::string iface) : m_nh(nh), m_loop_rate(100), m_iface(iface) {
    ...
    // PUBLISHERS
	m_can_rx_pub 	= m_nh.advertise<hwctrl2::CanFrame>("can_frames_rx", 128);

	// SUBSCRIBERS
	m_can_tx_sub 	= m_nh.subscribe("can_frames_tx", 128, &CanbusThread::can_tx_callback, this);
}
...
void CanbusThread::can_tx_callback(boost::shared_ptr<hwctrl2::CanFrame> frame) {
...
int CanbusThread::read_can_frames() {
    ...
    while((nbytes=read(m_sock, &rx_frame, sizeof(struct can_frame))) > 0) {
        auto frame_msg = boost::make_shared<hwctrl2::CanFrame>();
...
void CanbusThread::sleep() {
    m_loop_rate.sleep();
}

void CanbusThread::shutdown() {
    close(m_sock);
}
